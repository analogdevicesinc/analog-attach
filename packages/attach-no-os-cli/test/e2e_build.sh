#!/bin/bash
# End-to-end test: ADXL355 3-axis accelerometer over SPI on max32690.
#
# Exercises: the SPI stack (platform extra -> no-OS init_param -> device) and a
# UNION property (comm_init, set as "<member> <value>").

source "$(cd "$(dirname "$0")" && pwd)/e2e_common.sh"

# Overridable so e2e_build_in_tree.sh can reuse this device configuration under a
# different project name and layout without repeating it.
PROJECT_NAME="${PROJECT_NAME:-e2e_test_adxl355}"
TARGET_MCU="max32690"

configure_nodes() {
    # A device is a descriptor node (adxl355_device) paired with its init_param struct
    # (adxl355_ip); the init_param references the no-OS SPI init_param, which in turn
    # references the platform (Maxim) SPI init_param.
    $ATTACH_NOOS add --name max_spi_ip --key platforms/maxim/max32690/max_spi_init_param.yaml
    $ATTACH_NOOS add --name no_os_spi_ip --key no-os/spi/no_os_spi_init_param.yaml
    $ATTACH_NOOS add --name adxl355_ip --key devices/adxl355/adxl355_init_param.yaml
    $ATTACH_NOOS add --name adxl355_device --key devices/adxl355/adxl355.yaml

    # Configure the Maxim SPI init_param
    $ATTACH_NOOS update max_spi_ip vssel MXC_GPIO_VSSEL_VDDIOH
    $ATTACH_NOOS update max_spi_ip polarity SPI_SS_POL_LOW

    # Configure the no-OS SPI init_param (references the Maxim ops + extra)
    $ATTACH_NOOS update no_os_spi_ip device_id 1
    $ATTACH_NOOS update no_os_spi_ip max_speed_hz 1000000
    $ATTACH_NOOS update no_os_spi_ip chip_select 0
    $ATTACH_NOOS update no_os_spi_ip platform_ops max_spi_ops
    $ATTACH_NOOS update no_os_spi_ip extra max_spi_ip

    # Configure the ADXL355 init_param (comm_init is a union: <member> <value>)
    $ATTACH_NOOS update adxl355_ip comm_type ADXL355_SPI_COMM
    $ATTACH_NOOS update adxl355_ip dev_type ID_ADXL355
    $ATTACH_NOOS update adxl355_ip comm_init spi_init no_os_spi_ip

    # Point the descriptor at its init_param
    $ATTACH_NOOS update adxl355_device '$init_param' adxl355_ip
}

run_e2e
