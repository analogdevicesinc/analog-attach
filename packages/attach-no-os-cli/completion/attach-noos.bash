# Bash completion for attach-noos (Analog Attach CLI)
# Install: attach-noos completion install [--user]
#
# Requires an `attach-noos` that provides `attach-noos complete` (0.1.0+). If you
# upgrade attach-noos while a shell is open, re-source this file — an older cached
# copy calls a flag that no longer exists and silently completes nothing.

_attach_noos_complete() {
    local cur="${COMP_WORDS[COMP_CWORD]}"
    local completions

    # attach-noos complete <route> <partial> [prior tokens...] — the same entrypoint
    ...
    completions=$(attach-noos complete "${route}" "${cur}" "${tokens[@]}" 2>/dev/null)

    COMPREPLY=($(compgen -W "${completions}" -- "${cur}"))
}

# -o nosort preserves the order emitted by `attach-noos complete` (workfile values and
# command names first, --flags last) instead of bash re-sorting alphabetically.
complete -o nosort -F _attach_noos_complete attach-noos
