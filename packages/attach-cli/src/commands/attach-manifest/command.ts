        ...
        .action(async () => {
            const manifest = {
                ...
                commands: {
                    ...
                    "add": {
                        description: "Add a new node to the overlay. Positional key is the compatible string (e.g. adi,ad7124-8) — used as the node name unless --name <name[@unit]> overrides it. --label <label> assigns a DTS label to the new node, useful for referencing it in later commands. --to <parent> sets the parent node; accepted forms: bare label (spi0), sigil label (&spi0), absolute path (/soc/spi@7e204000), sigil path (&{/soc/spi@7e204000}), slash-separated label/child (spi0/ad7124@0/channel@0 or &spi0/ad7124@0), or multiple space-separated tokens resolved from root (soc spi@7e204000 ad7124@0 → /soc/spi@7e204000/ad7124@0). Fails with parent-not-found if the parent does not exist in either the base DTS or the overlay.",
                    ...
                    "update": {
                        description: "Set or insert a property on an overlay node (upsert). The last slash-delimited segment of the path is the property name; everything before it identifies the node. Accepted forms: node label + property (imu1/reg), sigil node label + property (&imu1/reg), absolute path + property (/soc/spi@7e204000/reg), sigil path + property (&{/soc/spi@7e204000}/reg), slash-separated label/child + property (imu1/channel@0/reg or &imu1/channel@0/reg), or space-separated tokens where the last token is the property (soc spi@7e204000 reg). --with <value> is required. Value syntax: single number 0, single string foo, boolean flag true/false, array [item1; item2; item3] (semicolon-separated, no trailing separator), matrix rows [a; b], [c; d].",
                        argv: ["attach-linux", "--json", "update"],
                    },
                    "delete": {
                        description: "Delete an overlay-added node (and its entire subtree) or a single property. Path identifies the target; accepted forms: node label (imu1), sigil node label (&imu1), absolute path (/soc/spi@7e204000), sigil path (&{/soc/spi@7e204000}), slash-separated label/child (imu1/channel@0 or &imu1/channel@0), space-separated tokens from root (soc spi@7e204000 → /soc/spi@7e204000). To delete a property use the slash-delimited forms where the last segment is the property name: imu1/reg, /soc/spi@7e204000/reg, imu1/channel@0/reg, &imu1/channel@0/reg, or space-separated with last token as property (soc spi@7e204000 reg). Without --force, a non-leaf node returns a preview (ok: true, severity: warn) with node_count and property_count — re-call with --force to confirm. Empty path without --force also previews; empty path with --force clears the entire overlay. Cannot delete base-tree nodes (returns in-base error).",
